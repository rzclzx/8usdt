class Deep {
  static deepStyle = {
    color1: '#1c2f37',  // 左侧数组颜色
    color2: '#3b1f34',  // 右侧数组颜色
    axisColor: '#212340', // 坐标轴颜色
    axisFontColor: '#666b93', // 坐标字体颜色
  ...
  constructor (canvas, toolTips, arr1, arr2, priceLabel = '委托价', sumLabel = '累计', bits) {
    this.canvas = canvas
  ...
  getPx1Arr (arr, startX, pxX) {
    this.arr1.map(v => {
      ...
      for (let i = 0; i <= parseInt((v[0] - startX) / pxX); i++) {
        arr[i][1] += Number(v[1])
      }
    })
  }
  ...
  getPx2Arr (arr, startX, pxX) {
    this.arr2.map(v => {
      for (let i = this.width; i > parseInt((v[0] - startX) / pxX); i--) {
        if (!arr[i]) break;
        arr[i][1] += Number(v[1])
      }
    })
