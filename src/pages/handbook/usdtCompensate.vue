  ...
  <div class="fund">
    ...
    <ul>
      <li :class="$route.params.id === item.id.toString() ? 'active' : ''"  v-for="(item, index) in contract" :key="index">
        <router-link :to="'/handbook/usdtCompensate/' + item.id">
    ...
    <st-row class="toHistory" justify="center" align="center">
      ...
      <router-link class="text2" to="/handbook/usdtInsurance/1001" v-html="$t('message.direct.navigation.secondaryTitle4')"></router-link>
    </st-row>
  </div>
</template>
